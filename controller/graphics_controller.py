...
class Graphics3DController:
    def __init__(self, graphics_3d, graphics_2d, view):
        ...
        self.view.projection_menu.setEnabled(True)
        self.view.side_menu.forward_label.setVisible(True)
        self.view.side_menu.backward_label.setVisible(True)
        self.view.side_menu.x_axis_label.setVisible(True)
        self.view.side_menu.x_axis_check.setVisible(True)
        self.view.side_menu.y_axis_label.setVisible(True)
    ...
    def object_rotate(self):
        ...
        if(selected != -1):
            if(y_diff != 0):
                axis = np.cross(self.graphic.vpn.coords[:3], self.graphic.vup.coords[:3])
                transformation = Rotation3D(Rotation3DType.OBJECT_CENTER, RotationAxis.U, axis, -y_diff, 0, 0, 0)
    ...
    def draw_objects(self, color, painter):
        objects_3d = self.graphic_3d.objects
        objects_2d = self.graphic_2d.objects
        line_clipping = self.graphic_3d.line_clipping
        ...
        if(not (color is None)):
            ...
            projection_normalization_matrix = self.graphic_3d.projection_normalization_matrix()
            viewport_transformation_matrix = self.graphic_3d.viewport_transformation_matrix()
            d = 1/10
            ...
            normalization_matrix = self.graphic_2d.normalization_matrix()
            viewport_transformation_matrix = self.graphic_2d.viewport_transformation_matrix()
    ...
    def zoom(self, step):
        self.erase()
        self.graphic_3d.zoom(step)
        self.graphic_2d.zoom(step)
        self.draw()
    ...
    def pan(self, axis, direction, step):
        ...
        self.graphic_3d.pan(axis, step*direction)
        
        if(axis != Axis.Z):
            self.graphic_2d.pan(axis, step*direction)
    ...
    def rotate(self, step, axis, direction):
        ...
        self.graphic_3d.rotate(axis, math.radians(degrees)*direction)
        if(axix == Axis.Z):
            self.graphic_2d.rotate(math.radians(degrees)*direction)
